...
function createConnection(pooled){
    ...
    //if(pooled){pooled.id = cid++;pooled.con = connection;pooled.status = PENDING; }else{pooled = {id: cid++,con: connection,status: PENDING };this.connections.push(pooled); }

    var handleError=function(err){self.log('connection closing because of error');console.log("ERR",err);
    connection.removeListener('end', endHandler);pooled.status = RETRY;pooled.con = undefined;
        if (pooled.timeout)clearTimeout(pooled.timeout);// pooled.timeout = setTimeout(createConnection.bind(self, pooled), self.retryDelay);
        self.emit('error', err);
    };

    connection.on('connect',function (err){console.log("err",err,"CON");self.log('connection connected: ' + pooled.id);
        if (self.drained) {self.log('connection closing because pool is drained');connection.close();return; }
        if (err){handleError(err);return; }
        var waiter = self.waiting.shift();
        if (waiter !== undefined)setUsed.call(self, pooled, waiter);else setFree.call(self, pooled);
    });

    connection.on('error', handleError);

    endHandler=function(){self.log('connection ended: '+pooled.id);if(self.drained)return;for(var i=self.connections.length-1;i>=0;i--){if(self.connections[i].con==connection){self.connections.splice(i,1);fill.call(self);return; } } };
